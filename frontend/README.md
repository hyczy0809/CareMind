...
- 后端暂不可用时，页面会显示“当前使用本地摘要”，并继续用本地 `attentionItems` 生成报告和 PDF。
- 导出的 PDF 会优先使用后端返回的摘要、问题清单和材料清单。

验收路径：

```text
智能记录保存一条照护记录 -> 进入复诊准备 -> 看到“复诊摘要已生成” -> 导出 PDF
```
...
- 复诊准备页切换“近 7 天 / 近 30 天”时，会先按 `careLogs.createdAt` 过滤，再生成核心指标、摘要、问题清单和 PDF。
...
```text
保存多条不同日期的照护记录 -> 进入复诊准备 -> 切换近 7 天 / 近 30 天 -> 核心指标、摘要条目、医生问题和导出 PDF 内容随范围变化
```
...
- 导出的 PDF 增加封面页，包含 CareMind 标识、患者昵称、时间范围、生成时间、记录数和核心指标。
- 导出流程会记录核心事件：`pdf_export_started`、`pdf_export_succeeded`、`pdf_export_failed`。
- 本地事件池会记录最多 100 条非隐私化事件，包括保存照护记录、更新行动状态、保存照护者状态、切换复诊范围、生成复诊摘要、导出 PDF 等。
- 设置页“核心事件”区域可查看最近 20 条事件，便于 demo 验收和调试。

验收路径：

```text
保存一条智能记录 -> 进入复诊准备 -> 导出复诊摘要 PDF -> 进入设置页 -> 看到 PDF 导出成功等核心事件
```
...
```text
进入复诊准备 -> 手动填写一条资料摘要 -> 只保存摘要 -> 看到复诊资料清单新增内容 -> 离开再返回仍保留 -> 导出 PDF 包含该资料
```
...
- PDF 会使用当前页面同一份摘要、问题清单和材料清单。

验收路径：

```text
复诊准备 -> 保存/确认一条复诊资料 -> 后端摘要显示“复诊摘要已同步” -> 复诊资料清单和导出 PDF 都包含该资料
```
